<script lang="ts">
  ...
  $: keyField = disableVirtualization ? 'id' : 'index'
  ...
  async function handle_scroll() {
    if (scrollElement && scrollElement.getBoundingClientRect().top <= viewport_height + 260) triggerNextPage()
...
<div
  ...
  <div
    ...
    <div class="thead top-0 table-header-group" class:sticky>
      <slot name="thead" />
    </div>

    {#each rendered as row (row[keyField])}
      <infinite-table-row class="group table-row">
